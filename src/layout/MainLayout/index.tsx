...
import { AppBar, Box, Container, CssBaseline, Snackbar, Toolbar, useMediaQuery, Alert } from '@mui/material';
...
const MainLayout = () => {
    ...
    const { setPermissions, user } = useAuth();
    ...
    useEffect(() => {
        if (user?.roleId) {
            const getPermissions = () => {
                PermissionService.getPermissionsAsync(user?.roleId).then((res) => {
                    setPermissions(res);
                });
            };
            getPermissions();
    ...
    const header = useMemo(
        () => (
            <Toolbar sx={{ p: condition ? '10px' : '16px', background: 'linear-gradient(180deg,#0094e7,#005e93)' }}>
                <Header />
            </Toolbar>
        ),
        // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    return (
        <Box sx={{ display: 'flex' }}>
            ...
            <Snackbar
                ...
                open={app.popupMessage.show}
                autoHideDuration={5000}
                onClose={handleClosePopupMessage}
            >
                <Alert variant="filled" onClose={handleClosePopupMessage} severity={'success'} sx={{ width: '100%' }}>
